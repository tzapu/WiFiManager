...
void WiFiManager::setupHTTPServer(){
  ...
  server->on(WM_G(R_root),       [this](AsyncWebServerRequest *request) { handleRoot(request); });
  server->on(WM_G(R_wifi),       [this](AsyncWebServerRequest *request) { handleWifi(request, true); });
  server->on(WM_G(R_wifinoscan), [this](AsyncWebServerRequest *request) { handleWifi(request, false); });
  server->on(WM_G(R_wifisave),   [this](AsyncWebServerRequest *request) { handleWifiSave(request); });
  server->on(WM_G(R_info),       [this](AsyncWebServerRequest *request) { handleInfo(request); });
  server->on(WM_G(R_param),      [this](AsyncWebServerRequest *request) { handleParam(request); });
  server->on(WM_G(R_paramsave),  [this](AsyncWebServerRequest *request) { handleParamSave(request); });
  server->on(WM_G(R_restart),    [this](AsyncWebServerRequest *request) { handleReset(request); });
  server->on(WM_G(R_exit),       [this](AsyncWebServerRequest *request) { handleExit(request); });
  server->on(WM_G(R_close),      [this](AsyncWebServerRequest *request) { handleClose(request); });
  server->on(WM_G(R_erase),      [this](AsyncWebServerRequest *request) { handleErase(request, false); });
  server->on(WM_G(R_status),     [this](AsyncWebServerRequest *request) { handleWiFiStatus(request); });
  server->onNotFound(            [this](AsyncWebServerRequest *request) { handleNotFound(request); });
  
  /* TODO no OTA for now :(
  server->on(WM_G(R_update), [this](AsyncWebServerRequest *request) { handleUpdate(request); });
  server->on(WM_G(R_updatedone), HTTP_POST, [this](AsyncWebServerRequest *request) { handleUpdateDone(request); }, [this](AsyncWebServerRequest *request) { handleUpdating(request); });
  */
...
void WiFiManager::HTTPSend(String content, AsyncWebServerRequest *request){
...
void WiFiManager::handleRequest(AsyncWebServerRequest *request) {
  ...
  if(!res){
    #ifndef WM_NOAUTH
    request->requestAuthentication(NULL, false); // DIGEST_AUTH
    #endif
...
const String &hostHeader(AsyncWebServerRequest *request) {
  static String empty;
...
boolean WiFiManager::captivePortal(AsyncWebServerRequest *request) {
  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_MAX,"-> " + hostHeader(request));
  #endif
  ...
  bool doredirect = serverLoc != hostHeader(request); // redirect if hostheader not server ip, prevent redirect loops
  ...
  if (doredirect) {
    ...
    request->redirect((String)F("http://") + serverLoc); // @HTTPHEAD send redirect
