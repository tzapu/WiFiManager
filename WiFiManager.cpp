...
void WiFiManagerParameter::init(const char *id, const char *label,
                                const char *defaultValue, int length,
...
WiFiManagerParameter::~WiFiManagerParameter() {
  ...
  _length = 0;  // setting length 0, ideally the entire parameter should be
                // removed, or added to wifimanager scope so it follows
}

// WiFiManagerParameter& WiFiManagerParameter::operator=(const
// WiFiManagerParameter& rhs){
...
const char *WiFiManagerParameter::getID() const { return _id; }
const char *WiFiManagerParameter::getPlaceholder() const { return _label; }
const char *WiFiManagerParameter::getLabel() const { return _label; }
int WiFiManagerParameter::getValueLength() const { return _length; }
...
bool WiFiManager::addParameter(WiFiManagerParameter *p) {
  ...
  if (p->getID()) {
    for (size_t i = 0; i < strlen(p->getID()); i++) {
      if (!(isAlphaNumeric(p->getID()[i])) && !(p->getID()[i] == '_')) {
        ...
        DEBUG_WM(
            DEBUG_ERROR,
            F("[ERROR] parameter IDs can only contain alpha numeric chars"));
#endif
        return false;
      }
    }
  ...
  if (_params == NULL) {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_DEV, F("allocating params bytes:"),
             _max_params * sizeof(WiFiManagerParameter *));
#endif
    _params = (WiFiManagerParameter **)malloc(_max_params *
  ...
  if (_paramsCount == _max_params) {
    ...
    DEBUG_WM(DEBUG_DEV, F("re-allocating params bytes:"),
             _max_params * sizeof(WiFiManagerParameter *));
#endif
    WiFiManagerParameter **new_params = (WiFiManagerParameter **)realloc(
        _params, _max_params * sizeof(WiFiManagerParameter *));
#ifdef WM_DEBUG_LEVEL
...
// DEBUG_WM(_max_params);
    ...
    if (new_params != NULL) {
      ...
      DEBUG_WM(DEBUG_ERROR,
               F("[ERROR] failed to realloc params, size not increased!"));
#endif
      return false;
    }
  }
...
void WiFiManager::_end() {
  _hasBegun = false;
  if (_userpersistent)
    WiFi.persistent(true);  // reenable persistent, there is no getter we rely
                            // on _userpersistent
...
// https://github.com/tzapu/WiFiManager/issues/1403
    ...
    // attempt to connect using saved settings, on fail fallback to AP config
    // portal
...
// handle failure mode Brownout detector etc.
    ...
    _usermode = WIFI_STA;  // When using autoconnect , assume the user wants sta
                           // mode on permanently.
...
// connected
      ...
      DEBUG_WM(DEBUG_VERBOSE, F("Connected in"),
               (String)((millis() - _startconn)) + " ms");
      DEBUG_WM(F("STA IP Address:"), WiFi.localIP());
#endif
      // Serial.println("Connected in " + (String)((millis()-_startconn)) + "
      // ms");
...
// #ifdef ESP8266MDNS_H
  ...
  // @note, this may have changed at some point, now it wont work, I have to set
  // it before.
  ...
  if (restart && (WiFi.status() == WL_CONNECTED)) {
    ...
    WiFi_Disconnect();
    delay(
        200);  // do not remove, need a delay for disconnect to change status()
  }
...
bool WiFiManager::startAP() {
  ...
  //@todo add callback here if needed to modify ap but cannot use
  // setAPStaticIPConfig
  //@todo rework wifi channelsync as it will work unpredictably when not
  // connected in sta
  ...
  // default channel is 1 here and in esplib, @todo just change to default
  // remove conditionals
  if (_apPassword != "") {
    if (channel > 0) {
      ret =
          WiFi.softAP(_apName.c_str(), _apPassword.c_str(), channel, _apHidden);
    } else {
      ret = WiFi.softAP(_apName.c_str(), _apPassword.c_str(), 1,
                        _apHidden);  // password option
    }
  } else {
#ifdef WM_DEBUG_LEVEL
...
boolean WiFiManager::configPortalHasTimeout() {
  ...
  if (_configPortalTimeout == 0 ||
      (_apClientCheck && (WiFi_softap_num_stations() > 0))) {
    // debug num clients every 30s
    if (millis() - timer > logintvl) {
      timer = millis();
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(DEBUG_VERBOSE, F("NUM CLIENTS: "),
               (String)WiFi_softap_num_stations());
#endif
    }
    _configPortalStart =
...
void WiFiManager::setupHTTPServer() {
  ...
  if (_httpPort != 80) {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("http server started with custom port: "),
             _httpPort);  // @todo not showing ip
#endif
  }

  server.reset(new WM_WebServer(_httpPort));
  // This is not the safest way to reset the webserver, it can cause crashes on
  // callbacks initilized before this and since its a shared pointer...
  ...
  server->on(WM_G(R_wifinoscan),
             std::bind(&WiFiManager::handleWifi, this, false));
...
void WiFiManager::setupDNSD() {
  ...
  DEBUG_WM(DEBUG_DEV, F("dns server started with ip: "),
           WiFi.softAPIP());  // @todo not showing ip
#endif
...
boolean WiFiManager::startConfigPortal(char const *apName,
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE,
             F("Starting Config Portal FAILED, is already running"));
#endif
    return false;
  }
  ...
  // HANDLE issues with STA connections, shutdown sta if not connected, or else
  // this will hang channel scanning and softap will not respond
  if (_disableSTA || (!WiFi.isConnected() && _disableSTAConn)) {
    // this fixes most ap problems, however, simply doing mode(WIFI_AP) does not
    // work if sta connection is hanging, must `wifi_station_disconnect`
    WiFi_Disconnect();
  ...
  bool result = connect = abort =
      false;  // loop flags, connect true success, abort true break
  uint8_t state;
...
// init configportal
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE,
             F("Config Portal Running, non blocking (processing)"));
    if (_configPortalTimeout > 0)
      DEBUG_WM(DEBUG_VERBOSE, F("Portal Timeout In"),
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_VERBOSE,
           F("Config Portal Running, blocking, waiting for clients..."));
  if (_configPortalTimeout > 0)
    DEBUG_WM(DEBUG_VERBOSE, F("Portal Timeout In"),
             (String)(_configPortalTimeout / 1000) + (String)F(" seconds"));
#endif
  ...
  while (1) {
    ...
    // I think.. this is to detect autoconnect by esp in background, there are
    // also many open issues about autoreconnect not working
...
uint8_t WiFiManager::processConfigPortal() {
  ...
  if (connect) {
    ...
    if (_shouldBreakAfterConfig) {
      // do save callback
      // @todo this is more of an exiting callback than a save, clarify when
      // this should actually occur confirm or verify data was saved to make
      // this more accurate callback
      if (_savewificallback != NULL) {
#ifdef WM_DEBUG_LEVEL
...
bool WiFiManager::shutdownConfigPortal() {
  ...
  // [APdisconnect] set_config failed! *WM: disconnect configportal -
  // softAPdisconnect failed still no way to reproduce reliably
  ...
  if (!ret)
    DEBUG_WM(DEBUG_ERROR,
             F("[ERROR] disconnect configportal - softAPdisconnect FAILED"));
  DEBUG_WM(DEBUG_VERBOSE, F("restoring usermode"), getModeString(_usermode));
#endif
  delay(1000);
  WiFi_Mode(_usermode);  // restore users wifi mode, BUG
                         // https://github.com/esp8266/Arduino/issues/4372
  if (WiFi.status() == WL_IDLE_STATUS) {
    WiFi.reconnect();  // restart wifi since we disconnected it in
                       // startconfigportal
#ifdef WM_DEBUG_LEVEL
...
uint8_t WiFiManager::connectWifi(String ssid, String pass, bool connect) {
  ...
  if (_cleanConnect)
    WiFi_Disconnect();  // disconnect before begin, in case anything is hung,
                        // this causes a 2 seconds delay for connect
  // @todo find out what status is when this is needed, can we detect it and
  // handle it, say in between states or idle_status to avoid these

  // if retry without delay (via begin()), the IDF is still busy even after
  // returning status E (5130) wifi:sta is connecting, return error
  ...
  while (retry <= _connectRetries && (connRes != WL_CONNECTED)) {
...
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(F("Connect Wifi, ATTEMPT #"),
               (String)retry + " of " + (String)_connectRetries);
#endif
    }
    ...
    if (ssid != "") {
      ...
      // _connectonsave is useful at all skip wait if not connecting
      // if(connect){
      if (_saveTimeout > 0) {
        connRes = waitForConnectResult(
            _saveTimeout);  // use default save timeout for saves to prevent
                            // bugs in esp->waitforconnectresult loop
      } else {
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("Connection result:"),
             getWLStatusString(connRes));
#endif
    retry++;
  }
...
// WPS enabled? https://github.com/esp8266/Arduino/pull/4889
  ...
  // do WPS, if WPS options enabled and not connected and no password was
  // supplied
...
bool WiFiManager::wifiConnectNew(String ssid, String pass, bool connect) {
  bool ret = false;
#ifdef WM_DEBUG_LEVEL
  // DEBUG_WM(DEBUG_DEV,F("CONNECTED: "),WiFi.status() == WL_CONNECTED ? "Y" :
  // "NO");
  DEBUG_WM(F("Connecting to NEW AP:"), ssid);
  DEBUG_WM(DEBUG_DEV, F("Using Password:"), pass);
#endif
  WiFi_enableSTA(
      true, storeSTAmode);  // storeSTAmode will also toggle STA on in default
                            // opmode (persistent) if true (default)
...
void WiFiManager::updateConxResult(uint8_t status) {
  ...
  // if(_lastconxresult == WL_CONNECT_FAILED){
  if (_lastconxresult == WL_CONNECT_FAILED ||
      _lastconxresult == WL_DISCONNECTED) {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_DEV, F("lastconxresulttmp:"),
             getWLStatusString(_lastconxresulttmp));
#endif
...
void WiFiManager::handleRequest() {
  ...
  // void requestAuthentication(HTTPAuthMethod mode = BASIC_AUTH, const char*
  // realm = NULL, const String& authFailMsg = String("") );
...
void WiFiManager::handleRoot() {
  ...
  handleRequest();
  String page =
      getHTTPHead(_title);  // @token options @todo replace options with title
  ...
  str.replace(
      ...
      configPortalActive
          ? _apName
          : (getWiFiHostname() + " - " +
             WiFi.localIP().toString()));  // use ip if ap is not active for
                                           // heading @todo use hostname?
  page += str;
  ...
  // @todo buggy, captive portals make a query on every page load, causing this
  // to run every time in addition to the real page load I dont understand why,
  // when you are already in the captive portal, I guess they want to know that
  // its still up and not done or gone if we can detect these and ignore them
  // that would be great, since they come from the captive portal redirect maybe
  // there is a refferer
}
...
#endif
    WiFi_scanNetworks(server->hasArg(F("refresh")),
                      false);  // wifiscan, force if arg refresh
    page += getScanItemOut();
  }
  String pitem = "";
...
String WiFiManager::getMenuOut() {
  ...
  for (auto menuId : _menuIds) {
    if ((String)_menutokens[menuId] == "param" && _paramsCount == 0)
      continue;  // no params set, omit params from menu, @todo this may be
                 // undesired by someone, use only menu to force?
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_VERBOSE, F("WiFi Scan ASYNC completed"),
           "in " + (String)(_lastscan - _startscan) + " ms");
  ...
  if (force || (_lastscan > 0 && (millis() - _lastscan > 60000))) {
    ...
    if (async && _asyncScan) {
      ...
      WiFi.scanNetworksAsync(
          std::bind(&WiFiManager::WiFi_scanComplete, this, _1));
#else
    ...
    _lastscan = millis();
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("WiFi Scan completed"),
             "in " + (String)(_lastscan - _startscan) + " ms");
#endif
    return true;
  } else {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("Scan is cached"),
             (String)(millis() - _lastscan) + " ms ago");
#endif
  }
  return false;
}
...
String WiFiManager::WiFiManager::getScanItemOut() {
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      DEBUG_WM(
          DEBUG_VERBOSE, F("AP: "),
          (String)WiFi.RSSI(indices[i]) + " " + (String)WiFi.SSID(indices[i]));
#endif
      ...
      if (_minimumQuality == -1 || _minimumQuality < rssiperc) {
        ...
        if (WiFi.SSID(indices[i]) == "") {
          ...
          continue;  // No idea why I am seeing these, lets just skip them for
                     // now
        }
        item.replace(FPSTR(T_V),
                     htmlEntities(WiFi.SSID(indices[i])));  // ssid no encoding
        item.replace(FPSTR(T_v), htmlEntities(WiFi.SSID(indices[i]),
                                              true));  // ssid no encoding
        ...
        if (tok_q)
          item.replace(FPSTR(T_q),
                       (String) int(round(
                           map(rssiperc, 0, 100, 1, 4))));  // quality icon 1-4
        if (tok_i) {
...
String WiFiManager::getStaticOut() {
  ...
  if ((_staShowStaticFields || _sta_static_ip) && _staShowStaticFields >= 0) {
    ...
    // @todo how can we get these accurate settings from memory ,
    // wifi_get_ip_info does not seem to reveal if struct ip_info is static or
    // not
    page += getIpForm(
        FPSTR(S_ip), FPSTR(S_staticip),
        (_sta_static_ip ? _sta_static_ip.toString() : ""));  // @token staticip
    // WiFi.localIP().toString();
    page += getIpForm(
        FPSTR(S_gw), FPSTR(S_staticgw),
        (_sta_static_gw ? _sta_static_gw.toString() : ""));  // @token staticgw
    // WiFi.gatewayIP().toString();
    page += getIpForm(
        FPSTR(S_sn), FPSTR(S_subnet),
        (_sta_static_sn ? _sta_static_sn.toString() : ""));  // @token subnet
    // WiFi.subnetMask().toString();
  }

  if ((_staShowDns || _sta_static_dns) && _staShowDns >= 0) {
    page += getIpForm(
        FPSTR(S_dns), FPSTR(S_staticdns),
        (_sta_static_dns ? _sta_static_dns.toString() : ""));  // @token dns
  }
  ...
  if (_paramsCount > 0) {
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      if (_params[i] == NULL || _params[i]->_length == 0 ||
          _params[i]->_length > 99999) {
// try to detect param scope issues, doesnt always catch but works ok
#ifdef WM_DEBUG_LEVEL
        DEBUG_WM(DEBUG_ERROR,
                 F("[ERROR] WiFiManagerParameter is out of scope"));
#endif
        return "";
      }
    }
    ...
    for (int i = 0; i < _paramsCount; i++) {
      // label before or after, @todo this could be done via floats or CSS and
      // eliminated
      String pitem;
      ...
      if (_params[i]->getID() != NULL) {
        if (tok_I)
          pitem.replace(FPSTR(T_I), (String)FPSTR(S_parampre) +
                                        (String)i);  // T_I id number
        if (tok_i)
        ...
        if (tok_p)
          pitem.replace(FPSTR(T_p),
                        FPSTR(T_t));  // T_p replace legacy placeholder token
        if (tok_t)
        ...
        if (tok_c)
          pitem.replace(
              FPSTR(T_c),
              _params[i]
                  ->getCustomHTML());  // T_c meant for additional attributes,
                                       // not html, but can stuff
      } else {
...
void WiFiManager::handleWifiSave() {
  ...
  DEBUG_WM(DEBUG_DEV, F("Method:"),
           server->method() == HTTP_GET ? (String)FPSTR(S_GET)
                                        : (String)FPSTR(S_POST));
#endif
  handleRequest();
  ...
  server->sendHeader(FPSTR(HTTP_HEAD_CORS),
                     FPSTR(HTTP_HEAD_CORS_ALLOW_ALL));  // @HTTPHEAD send cors
  HTTPSend(page);
  ...
  connect =
      true;  // signal ready to connect/reset process in processConfigPortal
}
...
void WiFiManager::handleParamSave() {
  ...
  String page =
      getHTTPHead(FPSTR(S_titleparamsaved));  // @token titleparamsaved
  page += FPSTR(HTTP_PARAMSAVED);
  page += FPSTR(HTTP_END);
...
void WiFiManager::doParamSave() {
  ...
  if (_paramsCount > 0) {
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      value.toCharArray(_params[i]->_value,
                        _params[i]->_length + 1);  // length+1 null terminated
#ifdef WM_DEBUG_LEVEL
...
//@todo wrap in build flag to remove all info code for memory saving
  ...
  infos = 28;
  String infoids[] = {
      F("esphead"),   F("uptime"),    F("chipid"),    F("fchipid"),
      F("idesize"),   F("flashsize"), F("corever"),   F("bootver"),
      F("cpufreq"),   F("freeheap"),  F("memsketch"), F("memsmeter"),
      F("lastreset"), F("wifihead"),  F("conx"),      F("stassid"),
      F("staip"),     F("stagw"),     F("stasub"),    F("dnss"),
      F("host"),      F("stamac"),    F("autoconx"),  F("wifiaphead"),
      F("apssid"),    F("apip"),      F("apbssid"),   F("apmac")};

#elif defined(ESP32)
  // add esp_chip_info ?
  infos = 26;
  String infoids[] = {
      F("esphead"),   F("uptime"),     F("chipid"),    F("chiprev"),
      F("idesize"),   F("flashsize"),  F("cpufreq"),   F("freeheap"),
      F("memsketch"), F("memsmeter"),  F("lastreset"), F("wifihead"),
      F("conx"),      F("stassid"),    F("staip"),     F("stagw"),
      F("stasub"),    F("dnss"),       F("host"),      F("stamac"),
      F("apssid"),    F("wifiaphead"), F("apip"),      F("apmac"),
      F("aphost"),    F("apbssid")
      // F("temp")
  };
#endif
...
String WiFiManager::getInfoData(String id) {
  ...
  if (id == F("esphead"))
    ...
    String revb = (String)(REG_READ(EFUSE_BLK0_RDATA3_REG) >>
                               (EFUSE_RD_CHIP_VER_RESERVE_S) &&
                           EFUSE_RD_CHIP_VER_RESERVE_V);
  ...
  // softAPBroadcastIP
    ...
    p.replace(FPSTR(T_1),
              WiFi.getAutoConnect() ? FPSTR(S_enable) : FPSTR(S_disable));
  }
#endif
#if defined(ESP32) && !defined(WM_NOTEMP)
  else if (id == F("temp")) {
    // temperature is not calibrated, varying large offsets are present, use for
    // relative temp changes only
...
/**
 * HTTPD CALLBACK exit, closes configportal if blocking, if non blocking
 * undefined
 */
void WiFiManager::handleExit() {
  ...
  server->sendHeader(
      F("Cache-Control"),
      F("no-cache, no-store, must-revalidate"));  // @HTTPHEAD send cache
  HTTPSend(page);
  delay(2000);
  abort = true;
}
...
boolean WiFiManager::captivePortal() {
  ...
  if (!_enableCaptivePortal)
    return false;  // skip redirections, @todo maybe allow redirection even when
                   // no cp ? might be useful

  String serverLoc = toStringIp(server->client().localIP());
  if (_httpPort != 80)
    serverLoc += ":" + (String)_httpPort;  // add port if not default
  bool doredirect =
      serverLoc != server->hostHeader();  // redirect if hostheader not server
                                          // ip, prevent redirect loops
...
void WiFiManager::stopCaptivePortal() {
  ...
  // @todo maybe disable configportaltimeout(optional), or just provide callback
  // for user
}
...
void WiFiManager::reportStatus(String &page) {
  // updateConxResult(WiFi.status()); // @todo: this defeats the purpose of last
  // result, update elsewhere or add logic here
  DEBUG_WM(DEBUG_DEV, F("[WIFI] reportStatus prev:"),
           getWLStatusString(_lastconxresult));
  DEBUG_WM(DEBUG_DEV, F("[WIFI] reportStatus current:"),
           getWLStatusString(WiFi.status()));
  String str;
  if (WiFi_SSID() != "") {
    if (WiFi.status() == WL_CONNECTED) {
...
bool WiFiManager::erase(bool opt) {
  ...
  if (opt) {
    ...
    err = nvs_flash_init();
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("nvs_flash_init: "),
             err != ESP_OK ? (String)err : "Success");
#endif
    err = nvs_flash_erase();
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("nvs_flash_erase: "),
             err != ESP_OK ? (String)err : "Success");
#endif
    return err == ESP_OK;
  }
...
/**
 * setWebServerCallback, set a callback after webserver is reset, and before
 * routes are setup if we set webserver handlers before wm, they are used and wm
 * is not by esp webserver on events cannot be overrided once set, and are not
 * mutiples
 * @access public
...
/**
 * setSaveParamsCallback, set a save params callback on params save in wifi or
 * params pages
 * @access public
 * @param {[type]} void (*func)(void)
 */
...
/**
 * setPreSaveParamsCallback, set a pre save params callback on params save prior
 * to anything else
 * @access public
 * @param {[type]} void (*func)(void)
 */
 ...
 * if enabled, then the configportal will enter a blocking loop and wait for
 * configuration if disabled use with process() to manually process webserver
 * @since $dev
 * @access public
 ...
 * sets ESP wifi.persistent so we can remember it and restore user preference on
 * destruct there is no getter in esp8266 platform prior to
 * https://github.com/esp8266/Arduino/pull/3857
 * @since $dev
 * @access public
 ...
 * if enabled, then the static ip, gateway, subnet fields will be visible, even
 * if not set in code
 * @since $dev
 * @access public
 ...
 * if enabled, then devices that use captive portal checks will be redirected to
 * root if not you will automatically have to navigate to ip [192.168.4.1]
 * @since $dev
 * @access public
 ...
 * On esp32 this is handled on SYSTEM_EVENT_STA_DISCONNECTED since it does not
 * exist in core yet
 * @since $dev
 * @access public
 * @param boolean enabled [true]
 ...
 * if enabled, then the configportal will start timeout when no stations are
 * connected to softAP disabled by default as rogue stations can keep it open if
 * there is no auth
 * @since $dev
 * @access public
 ...
 * if enabled, then the configportal will restart timeout when client requests
 * come in
 * @since $dev
 * @access public
 * @param boolean enabled [true]
 ...
 * @since $dev
 * @access public
 * @param  char* hostname 32 character hostname to use for sta+ap in esp32, sta
 * in esp8266
 ...
 * if param is present in menu , params will be removed from wifi page
 * automatically eg. const char * menu[] = {"wifi","setup","sep","info","exit"};
...
// frag);
#elif defined ESP32
    // total_free_bytes;      ///<  Total free bytes in the heap. Equivalent to
    // multi_free_heap_size(). total_allocated_bytes; ///<  Total bytes
    // allocated to data in the heap. largest_free_block;    ///<  Size of
    // largest free block in the heap. This is the largest malloc-able size.
    // minimum_free_bytes;    ///<  Lifetime minimum free heap size. Equivalent
    // to multi_minimum_free_heap_size(). allocated_blocks;      ///<  Number of
    // (variable size) blocks allocated in the heap. free_blocks;           ///<
    // Number of (variable size) free blocks in the heap. total_blocks; ///<
...
// DEBUG_WM(printf("Chip Info: Model: %d, cores: %d, revision: %d",
// chipInfo.model.c_str(), chipInfo.cores, chipInfo.revision)); DEBUG_WM("Chip
...
#endif
      _apPassword = "";
      return false;  // @todo FATAL or fallback to empty , currently fatal, fail
                     // secure.
    }
#ifdef WM_DEBUG_LEVEL
...
bool WiFiManager::WiFiSetCountry() {
  ...
  // @todo move these definitions, and out of cpp
  // `esp_wifi_set_country(&WM_COUNTRY_US)`
  bool ret = true;
...
bool WiFiManager::WiFi_Mode(WiFiMode_t m, bool persistent) {
  ...
  ret = WiFi.mode(
      m);  // @todo persistent check persistant mode, was eventually added to
           // esp lib, but have to add version checking probably
...
bool WiFiManager::WiFi_enableSTA(bool enable, bool persistent) {
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_DEV, F("WiFi_enableSTA"),
           (String)enable ? "enable" : "disable");
#endif
#ifdef ESP8266
  WiFiMode_t newMode;
  ...
  if (persistent && esp32persistent) WiFi.persistent(true);
  ret = WiFi.enableSTA(
      enable);  // @todo handle persistent when it is implemented in platform
  if (persistent && esp32persistent) WiFi.persistent(false);
  return ret;
#endif
}
...
#endif
    // Serial.println(F("\n[EVENT] WiFiEvent logging (wm debug not
    // available)")); Serial.print(F("[EVENT] ID: ")); Serial.println(event);
    return;
  }
#ifdef WM_DEBUG_LEVEL
// DEBUG_WM(DEBUG_VERBOSE,"[EVENT]",event);
  ...
  if (event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED) {
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE, F("[EVENT] WIFI_REASON: "),
    ...
    if (info.wifi_sta_disconnected.reason == WIFI_REASON_AUTH_EXPIRE ||
      ...
      _lastconxresulttmp =
          7;  // hack in wrong password internally, sdk emit
              // WIFI_REASON_AUTH_EXPIRE on some routers on auth_fail
    } else
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(DEBUG_VERBOSE,
             F("[Event] SYSTEM_EVENT_STA_DISCONNECTED, reconnecting"));
#endif
    WiFi.reconnect();
#endif
...
void WiFiManager::handleUpdate() {
  ...
  str.replace(
      ...
      configPortalActive
          ...
          : (getWiFiHostname() + " - " +
             WiFi.localIP()
                 .toString()));  // use ip if ap is not active for heading
  page += str;
...
void WiFiManager::handleUpdating() {
  // @todo
  // cannot upload files in captive portal, file select is not allowed, show
  // message with link or hide cannot upload if softreset after upload, maybe
  // check for hard reset at least for dev, ERROR[11]: Invalid bootstrapping
  // state, reset ESP8266 before updating add upload status to webpage somehow
  ...
  // combine route handlers into one callback and use argument or post checking
  // instead of mutiple functions maybe, if POST process else server upload
  // page? [x] add upload checking, do we need too check file? convert output to
  // debugger if not moving to example
  if (captivePortal())
    return;  // If captive portal redirect instead of displaying the page
  bool error = false;
  unsigned long _configPortalTimeoutSAV =
      _configPortalTimeout;  // store cp timeout
...
#elif defined(ESP32)
    // Think we do not need to stop WiFIUDP because we haven't started a
    // listener maxSketchSpace = (ESP.getFlashChipSize() - 0x1000) & 0xFFFFF000;
...
#endif
      error = true;
      Update.end();  // Not sure the best way to abort, I think client will keep
                     // sending..
    }
  }
  // UPLOAD WRITE
...
#ifdef WM_DEBUG_LEVEL
      DEBUG_WM(DEBUG_ERROR, F("[ERROR] OTA Update WRITE ERROR"),
               Update.getError());
